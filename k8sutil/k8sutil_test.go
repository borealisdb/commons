...
func TestSameService(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			current: newsService(
				map[string]string{
					BorealisDNSNameAnnotation: "clstr.borealisdb.io",
					ElbTimeoutAnnotationName:  ElbTimeoutAnnotationValue,
				},
		...
		},
		{
			...
			new: newsService(
				map[string]string{
					BorealisDNSNameAnnotation: "new_clstr.borealisdb.io",
					ElbTimeoutAnnotationName:  ElbTimeoutAnnotationValue,
				},
				v1.ServiceTypeLoadBalancer,
				[]string{"128.141.0.0/16", "137.138.0.0/16"}),
			match:  false,
			reason: `new service's annotations does not match the current one: 'external-dns.alpha.kubernetes.io/hostname' changed from 'clstr.borealisdb.io' to 'new_clstr.borealisdb.io'.`,
		},
		{
